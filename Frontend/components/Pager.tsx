        ...
        p === -1
          ...
          : (
            <button key={p} onClick={() => onPageChange(p)}
              className={`min-w-[32px] h-[32px] text-xs font-semibold rounded-lg transition-colors ${p === safePage ? 'bg-green-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100 border border-gray-200'}`}>
              {p}
            </button>
          )
      )}
