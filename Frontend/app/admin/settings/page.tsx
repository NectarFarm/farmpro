...
import { Settings, Palette, Package2, BookCheck, Building2, Check, AlertTriangle } from 'lucide-react';
...
export default function AdminSettingsPage() {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setSettingsOk(true); setSettingsMsg(t('brandingSaved'));
    } catch (e) { setSettingsOk(false); setSettingsMsg((e as Error).message); } finally { setSettingsSaving(false); }
  };

  const onLogoFile = (file: File) => {
    if (file.size > 400_000) { setSettingsOk(false); setSettingsMsg(t('logoTooLarge')); return; }
    ...
    <div className="p-6 flex flex-col gap-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{t('settingsDesc')}</p>
        </div>
      </div>
