...
export default function OwnerDashboardPage() {
  ...
  const kpiCards = [
    ...
    { label:'Avg FCR', value: String(kpis.avgFCR), Icon: Wheat, tint:'text-violet-600 bg-violet-50', sub:'target ≤ 2.8', good:kpis.avgFCR <= 2.8 },
    ...
    <div className="p-6 flex flex-col gap-6 max-w-7xl">
      ...
      {loaded && kpis.activeBatches === 0 && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-5 flex items-center justify-between flex-wrap gap-3">
          ...
          <a href="/owner/farm" className="px-4 py-2 bg-green-600 text-white rounded-lg font-semibold text-sm shrink-0">Go to Farm →</a>
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200/80 rounded-2xl p-5 shadow-sm">
        <div className="flex items-center gap-4 mb-4 flex-wrap">
          ...
          <span className="flex items-center gap-1.5 text-xs text-gray-500"><span className="w-2.5 h-2.5 rounded-sm bg-amber-400" />Revenue (KES)</span>
        </div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chart.data} margin={{ top:4, right:8, bottom:0, left:-8 }} barGap={2}>
                ...
                <Tooltip cursor={{ fill:'#f8fafc' }} contentStyle={{ borderRadius:12, border:'1px solid #e2e8f0', fontSize:12, boxShadow:'0 4px 12px rgba(0,0,0,0.06)' }}
                  formatter={(v, n) => n === 'revenue' ? [`KES ${Number(v).toLocaleString()}`, 'Revenue'] : [`${v}`, String(n)]} />
                {chart.products.map((p, i) => (
