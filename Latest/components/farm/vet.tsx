      ...
      <div className="px-screen" style={{ paddingTop: 12, paddingBottom: 40 }}>
        {employeeError && (
          <div className="farm-card" style={{ padding: '10px 14px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{employeeError}</span>
          </div>
        )}
        {error && (
          <div className="farm-card" style={{ padding: '10px 14px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {!loading && batches && batches.length === 0 && !error && (
          <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600 }}>No batches on this farm yet</div>
          </div>
        )}
              ...
              <div key={b.id} className="farm-card" style={{ padding: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace' }}>{b.code} · {b.species || b.enterprise}</div>
                  </div>
                  <span className={`chip ${deaths > 0 ? 'chip-critical' : 'chip-ok'}`} style={{ fontSize: 9 }}>{deaths} death{deaths === 1 ? '' : 's'}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 10 }}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>{b.currentQty}</div>
                    <div style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 600 }}>Current count</div>
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>{history.length}</div>
                    <div style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 600 }}>Mortality records</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '7px 0', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', background: isOpen ? 'rgba(74,222,128,0.1)' : 'var(--card)', border: '1px solid var(--border-subtle)', color: isOpen ? 'var(--primary-green)' : 'var(--text-muted)' }}
                  >
                  ...
                  <button
                    ...
                    style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '7px 0', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: employee ? 'pointer' : 'not-allowed', opacity: employee ? 1 : 0.5, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: 'var(--status-critical)' }}
                  >
                ...
                {isOpen && (
                  <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border-subtle)' }}>
                    {history.length === 0 ? (
                      <div style={{ fontSize: 11, color: 'var(--text-dim)', textAlign: 'center', padding: '6px 0' }}>No mortality records for this batch.</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {history.map((r) => (
                          <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, padding: '6px 8px', background: 'var(--surface)', borderRadius: 8 }}>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'flex-end', zIndex: 200 }} onClick={onClose}>
      <div style={{ background: 'var(--surface)', borderRadius: '20px 20px 0 0', padding: 20, width: '100%', border: '1px solid var(--border-subtle)' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15 }}>Log Mortality</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace' }}>{batch.code} · {batch.name}</div>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'center', marginBottom: 16 }}>
          <button type="button" onClick={() => setCount(Math.max(0, count - 1))} style={{ width: 48, height: 48, borderRadius: 14, background: 'var(--card)', border: '1px solid var(--border-subtle)', fontSize: 22, color: 'var(--text-primary)', cursor: 'pointer' }}>−</button>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 44, fontWeight: 700, color: count > 0 ? 'var(--status-critical)' : 'var(--text-primary)', lineHeight: 1 }}>{count}</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>deaths</div>
          </div>
          <button type="button" onClick={() => setCount(count + 1)} style={{ width: 48, height: 48, borderRadius: 14, background: 'var(--card)', border: '1px solid var(--border-subtle)', fontSize: 22, color: 'var(--text-primary)', cursor: 'pointer' }}>+</button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
          {CAUSES.map((c) => (
            <button
              ...
              type="button"
              onClick={() => setCause(c)}
              style={{ padding: '10px 8px', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', background: c === cause ? 'rgba(248,113,113,0.12)' : 'var(--card)', border: c === cause ? '1px solid rgba(248,113,113,0.3)' : '1px solid var(--border-subtle)', color: c === cause ? 'var(--status-critical)' : 'var(--text-muted)' }}
            >
              {c}
            </button>
          ))}
