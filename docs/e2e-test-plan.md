...
| A13 | **Highest-risk case in this plan.** Approving in Governance flips the request status and writes an audit row; nothing in the screen updates headcount, dashboard figures or reports. Any such effect is server-side only. | Approve a mortality or physical-count request, then check all of: the batch's headcount and ledger on Units, Finance's Batch P&L, and the Mortality/Production report. A silent failure here is invisible from Governance itself. |
...
| A18 | GL Dimensions: a dimension set to required ("Yes") on an account makes the server refuse postings that leave it empty; the "Blocked" chip means "Never analyse this account by this dimension". Values projected from farms/units/batches are deliberately locked, and the "Other defaults" tab auto-saves each select with no Save button and no confirmation. | Negative case: mark a dimension required on an account, then post without it and confirm the refusal. Do not file the locked projected values or the missing Save button as bugs. |
