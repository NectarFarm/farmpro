...
import { computeVaccinationReport, parseDateRange, InvalidDateRangeError, REPORT_VIEWER_ROLES, withReportCache } from '@/lib/reports'

// ── GET /api/reports/vaccination (issue #376 Gap 3) ─────────────────────────
// Derived from the real `records` table (`type: 'health'`) — see
// lib/reports.ts's computeVaccinationReport. Same role gate + session-only
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const report = await withReportCache('vaccination', tenantId, from, to, farmFilter ?? undefined, () => computeVaccinationReport(tenantId, from, to, farmFilter ?? undefined))
    return ok(report)
  } catch (err) {
